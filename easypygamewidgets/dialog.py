...
from easypygamewidgets.masterWidgets import Widget, Screenable, Deletable
...
class Dialog(Widget, Screenable, Deletable):
    ...
    def compute_auto_size(self):
        ...
        computed_w = max(total_title_w + self._title_alignment_spacing,
    ...
    def clone(self):
        ...
        if copied_widget._auto_size:
            ...
            copied_widget._rect = pygame.Rect(copied_widget._x, copied_widget._y,
    ...
    def configure(self, **kwargs: Unpack[TypeHints.DialogConfig]):
        ...
        self._needs_redraw = True
        self._needs_transform = True
        if any(k in kwargs for k in ('auto_size', 'x', 'y', 'width', 'height', 'title', 'description', 'title_font',
            ...
            if self._auto_size:
                self._width, self._height = self.compute_auto_size()
            self._rect = pygame.Rect(self._x, self._y, self._width, self._height)
        if any(k in kwargs for k in ('title_line_spacing', 'title_font', 'description_font',
...
def render_dialog_surface(dialog, is_hovering):
    ...
    if not dialog.hide_border and brd_color:
        pygame.draw.rect(cached, brd_color, local_rect,
    ...
    if not dialog.hide_text:
        render_text_block(cached, dialog.title, title_font, title_color, dialog.title_alignment,
                          dialog.title_alignment_spacing, dialog.title_line_spacing, title_rect)
        render_text_block(cached, dialog.description, desc_font, desc_color, dialog.description_alignment,
    ...
    if dialog.widgets:
        ...
        if not dialog.hide_text:
            ...
            if divider_color:
                pygame.draw.line(cached, divider_color, (local_rect.left + pad, divider_y),
